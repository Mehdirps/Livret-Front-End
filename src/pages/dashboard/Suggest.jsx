...
const Suggest = () => {
  ...
  useEffect(() => {
    fetch(process.env.REACT_APP_API_URL + "dashboard/suggestions", {
      ...
      .then((data) => {
        ...
        setSuggestionStats({
          accepted: data.suggestions.filter((s) => s.status === "accepted")
            .length,
          refused: data.suggestions.filter((s) => s.status === "refused")
            .length,
          pending: data.suggestions.filter((s) => s.status === "pending")
            .length,
        });
  ...
  return (
    <div className="container">
      ...
      <p>
        Retrouver ici, les suggestions faites par vos visiteurs pour améliorer
        votre livret.
      </p>
      <hr />
      ...
      <hr />
      <SuggestionFilter
        suggestionsToFiltered={suggestionsToFiltered}
