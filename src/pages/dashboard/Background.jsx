...
const Background = () => {
      ...
      .then((data) => {
        const groups = data.background_groups.map((group) => {
          const backgrounds = data.backgrounds.filter(
            (background) => background.backgrounds_group_id === group.id
          );
          return {
            ...group,
            backgrounds,
  ...
  const handleChooseBackground = (backgroundId) => {
    fetch(
      process.env.REACT_APP_API_URL + "dashboard/background/" + backgroundId,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    <div>
      ...
      <div className="row">
        <div className="col-md-4">
          ...
          {livret.background && (
            <img
              src={process.env.REACT_APP_URL + livret.background}
              alt="Fond actuel"
              className="img-fluid"
            />
          )}
      ...
      <div className="row w-100 d-flex justify-content-center">
        {backgroundGroups.map((group) => (
          <div className="col-md-3" key={group.id}>
            <div className="card">
              <button
                className="btn btn-primary btn_group"
                ...
                <div className="row">
                  {group.backgrounds.length > 0 ? (
                    group.backgrounds.map((background) => (
                      <div
                        className={"card col-md-4 backgroup_card"}
                        key={background.id}
                        onClick={() => handleChooseBackground(background.id)}
                      >
                        <img
                          src={process.env.REACT_APP_URL + background.path}
                          alt={background.name}
