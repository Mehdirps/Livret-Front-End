...
const FirstLogin = () => {
  ...
  const validate = () => {
    ...
    if (!establishmentType)
      newErrors.establishmentType = "Le type d'établissement est obligatoire";
    if (!establishmentName)
      newErrors.establishmentName = "Le nom de l'établissement est obligatoire";
    if (!establishmentAddress)
      newErrors.establishmentAddress =
        "L'adresse de l'établissement est obligatoire";
    if (!establishmentPhone)
      newErrors.establishmentPhone =
        "Le téléphone de l'établissement est obligatoire";
    if (!establishmentEmail)
      newErrors.establishmentEmail =
        "L'email de l'établissement est obligatoire";
    if (!establishmentWebsite)
      newErrors.establishmentWebsite =
    ...
    fetch(`${process.env.REACT_APP_API_URL}dashboard/first_login`, {
      ...
      .then((data) => {
        if (data.success) {
          ...
          setRequestError(
            data.error || data.jwt_error || "Une erreur est survenue"
          );
        }
      })
      .catch((error) => {
    ...
    <div className="container">
      <div className="row">
        <div className="col-12">
          ...
          <p>
            Vous êtes sur le point de créer votre livret d'accueil. Pour cela,
          ...
          <form onSubmit={handleSubmit} className="p-5 bg-light rounded">
            <div className="form-group">
              ...
              />
              {errors.livretName && (
                <div className="alert alert-danger">{errors.livretName}</div>
              )}
            </div>
            <div className="form-group">
              ...
              <div className="col-md-6">
                <div className="form-group mb-3">
                  <label htmlFor="establishment_phone" className="form-label">
                    <i className="bi bi-telephone"></i> Téléphone de
                    l'établissement
                  </label>
                  <input
                    type="text"
