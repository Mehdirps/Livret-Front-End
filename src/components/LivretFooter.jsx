...
const LivretFooter = ({ livret }) => {
  return (
    <footer className="container mb-5 livret-footer">
      {livret.suggest && <LivretSuggestionForm livretId={livret.id} />}
      <div className="row socials">
        {livret.establishmentWebsite && (
          <div className="social-link">
            <a
              href={livret.establishmentWebsite}
              target="_blank"
              rel="noopener noreferrer"
            >
              <i
                className="bi bi-globe website-icon"
                style={{ color: "#0a66c2" }}
              ></i>{" "}
              {/* Couleur pour le site web */}
            </a>
          </div>
        )}
        {livret.instagram && (
        ...
        {livret.twitter && (
          <div className="social-link">
            <a href={livret.twitter} target="_blank" rel="noopener noreferrer">
              <i
                className="bi bi-twitter twitter-icon"
                style={{ color: "#1DA1F2" }}
              ></i>
            </a>
          </div>
        ...
        {livret.tripadvisor && (
          <div className="social-link">
            <a
              href={livret.tripadvisor}
              target="_blank"
              rel="noopener noreferrer"
            >
              <i
                className="bi bi-globe tripadvisor-icon"
                style={{ color: "#34E0A1" }}
              ></i>{" "}
              {/* Couleur pour TripAdvisor */}
            </a>
          </div>
        )}
      </div>
